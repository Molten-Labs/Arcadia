...
import {
  ...
  TrendingUp, Shield, DollarSign, Settings, Briefcase,
  Crown, Terminal, Home,
} from "lucide-react";
...
/* Investor pages consolidated to 2: Dashboard (all-in-one) + Marketplace */
const INVESTOR_NAV = [
  { href: "/dashboard",   icon: LayoutDashboard, label: "Dashboard",    primary: true  },
...
/* Mobile nav for guests (minimal — Traders + Leaderboard + Home) */
const GUEST_MOBILE_NAV = [
...
function NavItem({
  ...
  return (
    ...
    >
      ...
      />
      <span className="text-xs font-semibold tracking-wide whitespace-nowrap" style={{ letterSpacing: "-0.01em" }}>
        {label}
      </span>
    </Link>
  );
...
export function Sidebar() {
  ...
  /* Desktop sidebar — only when connected */
  const showDesktopSidebar = mounted && connected;

  /* Mobile nav items — always shown, adapts to role */
  const mobileItems = mounted && connected
    ? [...primaryLinks.slice(0, 4), { href: "/settings", icon: Settings, label: "More" }]
    : GUEST_MOBILE_NAV;
  ...
  return (
    <>
      ...
      {/* Mobile bottom nav — always rendered (guests get 3 items, connected get 5) */}
      {mounted && (
        <nav
          className="fixed bottom-0 left-0 right-0 z-40 flex md:hidden pb-safe-bottom"

        ...
        <div className="surface rounded-2xl p-6 md:p-8 min-h-[360px]">
          ...
          <div className="flex justify-between mt-8 pt-6 border-t border-border">
            ...
            {step < steps.length - 1 ? (
              ...
              <Button onClick={() => setTx(true)} className="bg-gradient-ember text-white border-0">Create & fund vault</Button>
            )}
          </div>
        </div>
      </div>
      <TxModal open={tx} onOpenChange={(o) => { setTx(o); if (!o) { toast.success("Vault created!"); navigate("/manager"); } }} kind="create" />
    </Layout>
  );
};

const Row = ({ label, value }: any) => <div className="flex justify-between"><dt className="text-muted-foreground">{label}</dt><dd className="font-medium">{value}</dd></div>;

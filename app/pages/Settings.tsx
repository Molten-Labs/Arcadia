          ...
          <Section title="Display">
            <div className="space-y-3">
              ...
              <FieldRow label="Default role on login">
                <select value={role} onChange={(e) => setRole(e.target.value as any)} className="h-10 rounded-md border border-input bg-background px-3 text-sm">
...
const Row = ({ label, value }: any) => <div className="flex justify-between items-center text-sm mb-3"><span className="text-muted-foreground">{label}</span>{value}</div>;
const FieldRow = ({ label, children }: any) => <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">{label}</span>{children}</div>;

...
const MOCK_POSITIONS: TerminalPosition[] = [
    {
        pair: "SOL/USDC",
        side: "long" as const,
        size: 1240,
...
const MOCK_HISTORY: FillHistory[] = [
    {
        id: "h1",
        time: "Apr 22 · 14:22",
        pair: "SOL/USDC",
...
const Trade = () => {
    ...
    const [positions, setPositions] =
        useState<TerminalPosition[]>(MOCK_POSITIONS);
    const [orders, setOrders] = useState<OpenOrder[]>(MOCK_OPEN_ORDERS);
    const [history, setHistory] = useState<FillHistory[]>(MOCK_HISTORY);
    ...
    const handleSubmit = () => {
        ...
        if (!connected) {
            setValidationError("Connect a wallet before placing orders.");
            toast.error("Connect a wallet first");
            return;
        }

        if (role !== "trader") {
            setValidationError("Switch to trader mode to place orders.");
            toast.error("Trader mode required");
            return;
        }
        ...
        const parsedPrice =
            orderType === "market" ? market.price : Number(price);
        if (!Number.isFinite(parsedPrice) || parsedPrice <= 0) {
            setValidationError("Enter a valid positive limit price.");
            toast.error("Enter a valid limit price");
            return;
        }

        const notional = parsedAmount * parsedPrice;
        if (side === "buy" && notional > usdcBalance) {
            setValidationError(
                `Insufficient USDC balance. Available: ${usdcBalance.toLocaleString()} USDC.`,
            );
            toast.error("Insufficient USDC balance");
            return;
        }

        if (side === "sell" && parsedAmount > baseBalance) {
            setValidationError(
                `Insufficient ${market.base} balance. Available: ${baseBalance} ${market.base}.`,
            );
            toast.error(`Insufficient ${market.base} balance`);
            return;
        }

        const now = new Date();
        const orderId = `ord-${now.getTime()}`;
        const fillId = `fill-${now.getTime()}`;
            ...
            setHistory((items) => [
                {
                    ...
                    size: parsedAmount,
                    price: parsedPrice,
                    fee: Math.max(0.01, notional * 0.001),
                },
                ...items,
            ]);
            setTab("history");
        ...
        toast.success(`${side.toUpperCase()} ${orderType} order submitted`, {
            description: `${parsedAmount} ${market.base} @ ${orderType === "market" ? "market" : `$${parsedPrice.toLocaleString()}`}`,
        });
        setAmount("");
        setPrice("");
        setPctSlider(0);
    ...
    const closePosition = (pair: string) => {
        setPositions((items) => items.filter((p) => p.pair !== pair));
        toast.success("Position close queued", {
            description: `${pair} close order added to the execution queue.`,
        });
    };
    ...
    const cancelOrder = (id: string) => {
        ...
        toast.success("Order cancelled", {
            description: order
                ? `${order.side.toUpperCase()} ${order.pair} limit order removed.`
                : "Open order removed.",
        });
    };
        ...
        <Layout>
                ...
                <div className="border-b border-border bg-info/5">
                    <div className="container py-2.5 flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2 text-info">
                            ...
                            You're in <strong>investor mode</strong>. Switch to
                            trader to place orders, or browse vaults to deposit.
                        </div>
                        <Link
                            to="/vaults"
            ...
            <div className="border-b border-border bg-card/40 backdrop-blur sticky top-16 z-30">
                <div className="container py-3 flex items-center gap-6 overflow-x-auto">
                    ...
                    <Stat
                        label="Price"
                        value={`$${market.price.toLocaleString(undefined, { maximumFractionDigits: 6 })}`}
                        highlight={market.change24h >= 0 ? "up" : "down"}
                    />
                    <Stat
                        label="24h change"
                    ...
                    <Stat
                        label="24h high"
                        value={`$${market.high24h.toLocaleString()}`}
                    />
                    <Stat
                        label="24h low"
                        value={`$${market.low24h.toLocaleString()}`}
                    />
                    <Stat
                        label="24h volume"
                        value={`$${fmtUSD(market.vol24h, { compact: true })}`}
                    />
                </div>
            </div>
            ...
            <div className="container py-4">
                <div className="grid grid-cols-12 gap-3">
                        ...
                        <div className="space-y-0.5">
                            {MARKETS.map((m) => (
                                <button
                                    ...
                                    onClick={() => setMarket(m)}
                                    className={cn(
                                        "w-full min-h-10 grid grid-cols-[1fr_auto] gap-2 px-2 py-2 rounded-md text-left text-xs transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
                                        m.pair === market.pair
                                            ? "bg-secondary"
                        ...
                        <div className="surface rounded-xl p-4">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <div className="text-[11px] text-muted-foreground font-mono">
                                    ...
                                    · C{" "}
                                    <span className="text-success">184.23</span>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center border-b border-border px-3">
                                {(
                                    ...
                                    <button
                                        ...
                                        onClick={() => setTab(t)}
                                        className={cn(
                                            "h-10 px-4 py-3 text-xs font-medium uppercase tracking-wider transition-colors relative focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
                                            tab === t
                                                ? "text-foreground"
                                                : "text-muted-foreground hover:text-foreground",
                                        )}
                    ...
                    <aside className="col-span-12 lg:col-span-3 space-y-3">
                        <OrderBook
                            mid={market.price < 1000 ? market.price : 184.23}
                        />

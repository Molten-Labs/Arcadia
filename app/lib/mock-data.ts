...
function genEquityCurve(days: number, start: number, end: number): EquityPoint[] {
  ...
  for (let i = 0; i <= days; i++) {
    ...
    const value = start + (end - start) * t + noise * (end - start);
    pts.push({ ts: now - (days - i) * 86400, value: Math.max(0.5, value) });
  }
  return pts;
}
...
function genTrades(n: number, profile: string): TradeRecord[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    trades.push({
      ...
      sig: `${Math.random().toString(36).slice(2, 8)}…${Math.random().toString(36).slice(2, 6)}`,
    });
  }
  return trades;
}
...
export const MOCK_TRADERS: TraderProfile[] = [
  {
    ...
    capacity: { total: 500000, used: 387000 },
    aum: 387000,
    investors_count: 34,
  ...
  },
  {
    ...
    capacity: { total: 1000000, used: 742000 },
    aum: 742000,
    investors_count: 67,
  ...
  },
  {
    ...
    capacity: { total: 250000, used: 198000 },
    aum: 198000,
    investors_count: 21,
  ...
  },
  {
    ...
    capacity: { total: 100000, used: 34000 },
    aum: 34000,
    investors_count: 8,
  ...
  },
  {
    ...
    capacity: { total: 750000, used: 621000 },
    aum: 621000,
    investors_count: 54,
...
export function getVaultByProfile(profile: string): VaultInfo | undefined {
  ...
  return {
    ...
    hwm: nav + 50_000,
    status: "active",
    capacity_usd: trader.capacity.total,
